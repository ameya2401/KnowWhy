...
import { connectGoogleDrive } from "@/services/integrationApi";
...
export function GoogleDriveCallbackPage() {
  ...
  // state parameter passes the project_id
  ...
  useEffect(() => {
    ...
    async function handleCallback() {
      ...
      try {
        const redirect_uri = `${window.location.origin}/integrations/drive/callback`;
        ...
        if (active) {
          ...
          setErrorMsg(errorObj.response?.data?.detail ?? "Failed to connect to Google Drive.");
        }
      }
    }
          ...
          {status === "loading" && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Authorizing with Google and securing credentials...
              </p>
            </div>
          )}
